...
新 clone 的环境首次使用会自动下载模型（约 184MB）；若已有下载好的 `backend/data/models` 目录，
直接复制到新环境即可免下载、完全离线。

Web 界面左下角「设置」面板可动态配置向量模型（无需重启）：向量来源（本地 / 远程接口 / 离线词法）、
远程 API Base URL / Key / 模型名。本地模型不可用且已配置远程接口时，记忆召回自动改用远程向量模型兜底。
